...
export default function HomePage() {
  const slides = [
    ...
    {
      image: AutomobilesImg,
      text: "AUTOMOBILE ONLINE AUCTION",
      type: "automobile",
    },
    {
      image: RealEstateImg,
      text: "REAL ESTATE ONLINE AUCTION",
      type: "realstate",
    },
  ];
    ...
    const run = async () => {
      try {
        ...
        const [autoRes, realRes, bizRes] = await Promise.all([
          fetch(`${base}/api/automobile`, {
            headers: { "Content-Type": "application/json" },
            cache: "no-store",
          }),
          fetch(`${base}/api/realestate`, {
            headers: { "Content-Type": "application/json" },
            cache: "no-store",
          }),
        ...
        if (autoRes.ok && autoData.success) {
          ...
          const filteredAutoList = (autoData.listings || []).filter((item) => {
            if (
              selectedCountry &&
              item.automobileCountry !== selectedCountry.split("|")[1]
            )
              return false;
            if (
              selectedState &&
              item.automobileState !== selectedState.split("|")[1]
            )
              return false;
            if (selectedCity && item.automobileCity !== selectedCity)
              return false;
            return true;
          });
        ...
        if (realRes.ok && realData.success) {
          ...
          const filteredRealList = (realData.listings || []).filter((item) => {
            if (
              selectedCountry &&
              item.propertyCountry !== selectedCountry.split("|")[1]
            )
              return false;
            if (
              selectedState &&
              item.propertyState !== selectedState.split("|")[1]
            )
              return false;
            if (selectedCity && item.propertyCity !== selectedCity)
              return false;
            return true;
          });
        ...
        if (bizRes.ok && bizData.success) {
          ...
          const filteredBizList = (bizData.listings || []).filter((item) => {
            ...
            if (
              ...
              return false;
            if (selectedCity && item.businessCity !== selectedCity)
              return false;
            return true;
          });
  ...
  /* filter all types of listings by country, state and city saved in local storage */
  useEffect(() => {
    console.log("HomePage - App State:", {
      selectedCountry,
      selectedState,
      selectedCity,
    });
    setAutoList(
      allAutoList.filter((item) => {
        if (
          selectedCountry &&
          item.automobileCountry !== selectedCountry.split("|")[1]
        )
    ...
    setRealList(
      allRealList.filter((item) => {
        if (
          selectedCountry &&
          item.propertyCountry !== selectedCountry.split("|")[1]
        )
          return false;
        if (selectedState && item.propertyState !== selectedState.split("|")[1])
          return false;
  ...
  useEffect(() => {
    ...
    if (realList.length === 0 && autoList.length === 0 && bizList.length === 0)
      return;
  ...
  const homeListings: HomeItem[] = useMemo(() => {
    ...
    if (balanced.length < TARGET_TOTAL) {
      ...
      for (const item of leftovers) {
        ...
        if (
          !balanced.some(
            (b) => b.category === item.category && b.id === item.id,
          )
        ) {
          balanced.push(item);
        }
      ...
      <div className="transition-colors mt-[95px]">
        ...
        <section className="relative container custom-bottom-margin">
          ...
          >
            {slides.map((slide, idx) => (
              <SwiperSlide key={idx}>
                <div
                  onClick={() => openRegistration(slide.type)}
        ...
        <section className="container non-container2 custom-bottom-margin">
          ...
          <p className="section-desc">
            Discover top listings handpicked for you — from rare finds to
            high-value deals.
          </p>
          ...
          {loadingGrid ? (
            ...
            <div className="featured-slider-container relative">
              ...
              {(() => {
                ...
                const allAutomobileItems: FeaturedItem[] = autoList
                  .filter((item) => isFeaturedAndValid(item))
                  .filter((item) => isNotEnded(item.duration))
                  .slice(0, 3)
                  .map((item) => ({
                ...
                const allBusinessItems: FeaturedItem[] = bizList
                  .filter((item) => isFeaturedAndValid(item))
                  .filter((item) => isNotEnded(item.duration))
                  .slice(0, 3)
                  .map((item) => ({
                    ...
                    {displaySlides.map((slideItems, slideIndex) => (
                      <SwiperSlide key={`featured-slide-${slideIndex}`}>
                        <div className="ribbon-container">
                                  ...
                                  <div className="content-area">
                                    ...
                                    <div className="details">
                                      <div className="sub-container">
                                        ...
                                        <div
                                          className={`${getPriceColorClass(status)}`}
                                        >
                                          {(() => {
                          ...
                          {slideItems.length < 3 &&
                            Array.from({ length: 3 - slideItems.length }).map(
                              (_, index) => (
                                <div
                                  key={`empty-${slideIndex}-${index}`}
                                  className="product-card opacity-0 pointer-events-none"
                                >
          ...
          <div className="grid grid-cols-3 gap-6 mobile-view-category">
            <Link href="/buyer/automobile" className="category-link group">
              <Image
                ...
                alt="Automobile"
                fill
                className="category-img group-hover:opacity-90 group-hover:scale-95"
              />
              <div className="category-card">
                <h2 className="">Automobiles</h2>
              </div>
            </Link>
        ...
        <section className="container non-container custom-bottom-margin">
          <h2 className="section-title">
            Discover <span className="font-bold">Real Estate</span>
          </h2>
          <p className="section-desc">
            Browse and bid on a wide range of real estate properties with live
              ...
              {realList.slice(0, 8).map((listing) => {
                ...
                return (
                  <div
                    ...
                    onClick={() =>
                      handleListingClick(listing.id, "Real Estate")
                    }
                  >
                    <div className="top-container">
                      <span
                        ...
                        <div className="details">
                          ...
                          <div className="sub-container">
                            ...
                            {(() => {
                              ...
                              if (status === "End") {
                                return (
                                  <span className="status-end">Ended</span>
                                );
                              }
        ...
        <section className="container non-container custom-bottom-margin">
          <h2 className="section-title">
            Discover <span className="font-bold">Business</span>
          </h2>
          <p className="section-desc">
            Browse and bid on a wide range of business opportunities with live
            updates and transparent pricing.
          </p>
        ...
        <section className="container non-container custom-bottom-margin">
          <h2 className="section-title">
            Discover <span className="font-bold">Automobiles</span>
          </h2>
          <p className="section-desc">
            Browse and bid on a wide range of automobiles with live updates and
            transparent pricing.
          </p>
              ...
              {autoList.slice(0, 8).map((listing) => {
                ...
                return (
                  <div
                    ...
                    className="group product-card"
                    onClick={() =>
                      handleListingClick(listing.id, "Automobiles")
                    }
                  >
                    <div className="top-container">
                      <span
                  ...
                  case "business":
                    return (
                      <BusinessRegistrationForm
                        onClose={closePopup}
                        preselectedCategory="Business"
                      />
                    );
                  case "automobile":
                    return (
                      <AutomobileRegistrationForm
                        onClose={closePopup}
                        preselectedCategory="Automobiles"
                      />
                    );
                  case "realstate":
                    return (
                      <RealStateRegistrationForm
                        onClose={closePopup}
                        preselectedCategory="Real Estate"
                      />
                    );
                  default:
                    return null;
