...
const step1Schema = z.object({
  ...
  duration: z.string().refine(
    (val) => {
      ...
      return (
        dateTimePattern.test(startPart.trim()) &&
...
export default function AutomobileRegistrationForm({
  onClose,
  preselectedCategory,
  data,
  ...
  const [selectedCategory, setSelectedCategory] = useState(
    preselectedCategory || "Real State",
  );
  ...
  const [registrationType, setRegistrationType] = useState<"already" | "new">(
    "already",
  );
  ...
  useEffect(() => {
    if (startDate && startTime && endDate && endTime) {
      ...
      const durationString = formatDurationForStorage(
        startDate,
        startTime,
        endDate,
        endTime,
  ...
  useEffect(() => {
    if (watchedpState && availableStates.length > 0 && selectedCountryCode) {
      ...
      if (state) {
        ...
        const cities = CSCCity.getCitiesOfState(
          selectedCountryCode,
          state.isoCode,
        );
  ...
  useEffect(() => {
    if (watchedpCity && watchedpCountry) {
      const legacyCountry = locationData.countries.find(
        (c) => c.name === watchedpCountry,
      );
      if (legacyCountry) {
        const legacyState = legacyCountry.states.find(
          (s) => s.name === watchedpState,
        );
        if (legacyState) {
          const legacyCity = legacyState.cities.find(
            (c) => c.name === watchedpCity,
          );
          if (legacyCity) {
  ...
  useEffect(() => {
    ...
    if (data && data.automobileCountry) {
      ...
      const matchedCountry = allCountries.find(
        (c) => c.name === data.automobileCountry,
      );
      if (matchedCountry) {
          ...
          if (matchedState) {
            ...
            setTimeout(() => {
              const matchedCity = cities.find(
                (c) => c.name === data.automobileCity,
              );
              if (matchedCity) {
                setValue("automobileCity", matchedCity.name);
                const legacyCountry = locationData.countries.find(
                  (lc) => lc.name === data.automobileCountry,
                );
                const legacyState = legacyCountry?.states.find(
                  (ls) => ls.name === data.automobileState,
                );
                const legacyCity = legacyState?.cities.find(
                  (lc) => lc.name === data.automobileCity,
                );
                setValue(
                  "automobilePincode",
                  legacyCity?.pincode || data.automobilePincode || "",
                );
              }
            }, 100);
          }
  ...
  const handleNext = async () => {
    ...
    if (currentStep === 1) {
      ...
      if (registrationType === "already") {
        ...
        isValid =
          isValid && (await trigger(["name", "email", "password", "type"]));
      }
        ...
        if (errors.media) {
          firstError =
            ...
            (Array.from(document.querySelectorAll("label")).find((label) =>
              label.textContent?.includes("Upload media"),
            )?.parentElement as HTMLElement);
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (deletedFiles.length > 0) {
        for (const f of deletedFiles) {
          ...
          formData.append(
            "file",
            f.url.replace(
              `https://pub-925e11758c384112b352fa4ab58ba020.r2.dev/`,
              "",
            ),
          );
          ...
          await fetch(
            `${process.env.NEXT_PUBLIC_WRANGLER_API_URL}/api/uploads/`,
            {
              method: "DELETE",
      ...
      media.push(
        ...(adDetails?.media || []).filter(
          (m) => !uploadedFiles.find((f) => f.name === m.name),
        ),
      );
      ...
      const payload = {
        ...
        accidenthistory:
          data.accidenthistory || adDetails?.accidentHistory || "",
        ...
        automobileCountry:
          data.automobileCountry || adDetails?.automobileCountry || "",
        automobileState:
          data.automobileState || adDetails?.automobileState || "",
        automobileCity: data.automobileCity || adDetails?.automobileCity || "",
        automobilePincode:
          data.automobilePincode || adDetails?.automobilePincode || "",
        ...
        warrantydetails:
          data.warrantydetails || adDetails?.warrantyDetails || "",
      };
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_WRANGLER_API_URL}/api/automobile${isSellerUpdatePage ? "/" + adDetails?.id : ""}`,
        {
  ...
  //   console.log("COMPONENT MEDIA:", data?.media);
  return (
          ...
          {steps.map((step, index) => (
            ...
            >
              <div className="number-container">
                ...
                >
                  {step.number}
                  <span className="step-title step-ex">{step.title}</span>
                </div>
              </div>
          ...
          <div className="space-y-0">
            <div className="register-popup-subcontainer2">
              ...
              <div className="cont">
                ...
                />
                {errors.subcategory && (
                  <p className="error-text">{errors.subcategory.message}</p>
                )}
              </div>
              ...
              <div className="cont">
                ...
                <Dialog open={isCalendarOpen} onOpenChange={setIsCalendarOpen}>
                  <DialogTrigger asChild>
                    ...
                    >
                      {startDate && endDate
                        ? formatForDisplay(startDate, startTime) +
                          " to " +
                          formatForDisplay(endDate, endTime)
                        : startDate && !endDate
                          ? formatForDisplay(startDate, startTime) +
                            " - Select End Date"
                          : "Start - End"}
            ...
            <div className="mt-6" data-media-upload>
                      ...
                      return (
                        <div key={index} className="file-item group">
                          ...
                          <div className="absolute inset-0 flex items-start justify-end p-1 opacity-100 group-hover:opacity-100 transition-opacity">
                            <div
                              onClick={() =>
                                handleDeleteFileFromDb(file, index)
                              }
              ...
              <div>
                {uploadedFiles.length === 0 ? (
                  ...
                  <div className="file-container">
                    ...
                    {uploadedFiles.length < 10 && (
                      <div className="upload-files" onClick={handleUploadClick}>
                ...
                {errors.media && (
                  <p className="error-text">{errors.media.message}</p>
                )}
              </div>
            </div>
          </div>
              ...
              <div className="cont">
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    ...
                    >
                      <SelectTrigger
                        className={`custom-input ${errors.body ? "border-red-500" : ""}`}
                      >
              ...
              <div className="cont">
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    ...
                    >
                      <SelectTrigger
                        className={`custom-input ${errors.fuel ? "border-red-500" : ""}`}
                      >
                ...
                />
                {errors.fuel && (
                  <p className="error-text">{errors.fuel.message}</p>
                )}
              </div>
              <div className="cont">
                <Label className="custom-label">
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    ...
                    >
                      <SelectTrigger
                        className={`custom-input ${errors.transmission ? "border-red-500" : ""}`}
                      >
              ...
              <div className="cont">
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    ...
                    >
                      <SelectTrigger
                        className={`custom-input ${errors.drive ? "border-red-500" : ""}`}
                      >
              ...
              <div className="cont">
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    ...
                    >
                      <SelectTrigger
                        className={`custom-input ${errors.odometerUnit ? "border-red-500" : ""}`}
                      >
                ...
                />
                {errors.odometerUnit && (
                  <p className="error-text">{errors.odometerUnit.message}</p>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="space-y-0">
            <div className="register-popup-subcontainer">
              <div className="cont">
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    ...
                    >
                      <SelectTrigger
                        className={`custom-input ${errors.condition ? "border-red-500" : ""}`}
                      >
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="bellow-average">
                          Below Average
                        </SelectItem>
              ...
              <div className="cont">
                    ...
                    <Select
                      ...
                      onValueChange={(value: string) => {
                        ...
                        if (errors.accidenthistory)
                          clearErrors("accidenthistory");
                      }}
                    >
                      <SelectTrigger
                        className={`custom-input ${errors.accidenthistory ? "border-red-500" : ""}`}
                      >
                ...
                {errors.accidenthistory && (
                  <p className="error-text">{errors.accidenthistory.message}</p>
                )}
              </div>
            </div>
            {accidenthistory === "Yes" && (
              <div className="cont">
                ...
                />
                {errors.history && (
                  <p className="error-text">{errors.history.message}</p>
                )}
              </div>
            )}
            <div className="register-popup-subcontainer my-6">
              <div className="cont">
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    ...
                    >
                      <SelectTrigger
                        className={`custom-input ${errors.shistory ? "border-red-500" : ""}`}
                      >
                ...
                />
                {errors.shistory && (
                  <p className="error-text">{errors.shistory.message}</p>
                )}
              </div>
              <div className="cont">
                <Label className="custom-label">
              ...
              <div className="cont">
                ...
                />
                {errors.vnumber && (
                  <p className="error-text">{errors.vnumber.message}</p>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="space-y-0">
            <div className="register-popup-subcontainer">
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <Select
                      ...
                      onValueChange={(value: string) => {
                        field.onChange(value);
                        if (errors.automobileCountry)
                          clearErrors("automobileCountry");
                      }}
                    >
                      <SelectTrigger
                        className={`custom-input ${errors.automobileCountry ? "border-red-500" : ""}`}
                      >
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <Select
                      ...
                      onValueChange={(value: string) => {
                        field.onChange(value);
                        if (errors.automobileState)
                          clearErrors("automobileState");
                      }}
                      disabled={
                        !watchedpCountry || availableStates.length === 0
                      }
                    >
                      <SelectTrigger
                        className={`custom-input ${errors.automobileState ? "border-red-500" : ""}`}
                      >
              ...
              <div className="cont">
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <Select
                      ...
                      onValueChange={(value: string) => {
                        field.onChange(value);
                        if (errors.automobileCity)
                          clearErrors("automobileCity");
                      }}
                      disabled={!watchedpState || availableCities.length === 0}
                    >
                      <SelectTrigger
                        className={`custom-input ${errors.automobileCity ? "border-red-500" : ""}`}
                      >
                ...
                />
                {errors.automobileCity && (
                  <p className="error-text">{errors.automobileCity.message}</p>
                )}
              </div>
              <div className="cont">
                ...
                {errors.automobilePincode && (
                  <p className="error-text">
                    {errors.automobilePincode.message}
                  </p>
                )}
              </div>
            </div>
        ...
        {currentStep === 5 && (
          <div className="space-y-0">
            <div className="register-popup-subcontainer">
              ...
              <div className="cont">
                <Label className="custom-label">Negotiable</Label>
                <Controller
                  name="negotiable"
                  control={control}
                  render={({ field }) => (
                    ...
                    >
                      <SelectTrigger
                        className={`custom-input ${errors.negotiable ? "border-red-500" : ""}`}
                      >
              ...
              <div className="cont mt-6">
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    ...
                    >
                      <SelectTrigger
                        className={`custom-input ${errors.warranty ? "border-red-500" : ""}`}
                      >
                ...
                />
                {errors.warranty && (
                  <p className="error-text">{errors.warranty.message}</p>
                )}
              </div>
              {warranty === "Yes" && (
                <div className="cont">
                  ...
                  {errors.warrantydetails && (
                    <p className="error-text">
                      {errors.warrantydetails.message}
                    </p>
                  )}
                </div>
              )}
              ...
              <div className="flex items-center space-x-4">
                <div className="flex items-center">
                  <input
                    ...
                    onChange={(e) =>
                      handleRegistrationTypeChange(e.target.value as "already")
                    }
                  />
                ...
                <div className="flex items-center">
                  <input
                    ...
                    onChange={(e) =>
                      handleRegistrationTypeChange(e.target.value as "new")
                    }
                  />
            ...
            <div className="text-center">
              ...
              <p className="msg-text">
                <span className="block">
                  Your verification link is on its way ✉️
                </span>
                <span className="block">
                  Tap the link in your inbox to verify
                </span>
              </p>
              <Button
                onClick={() => {
