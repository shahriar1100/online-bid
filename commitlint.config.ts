...
const Configuration: UserConfig = {
  ...
  rules: {
    // Allow JIRA ticket prefix before conventional commit format
    "subject-case": [0], // Disable subject case checking
    "header-max-length": [2, "always", 100], // Increase header length for JIRA prefix
  },
  parserPreset: {
    parserOpts: {
      // Custom parser to handle JIRA ticket prefix
      headerPattern: /^(?:(AUC-\d+) \| )?(\w*)(?:\(([^)]*)\))?: (.*)$/,
